contract StorageLayoutMapping {
    uint x = 2;
    mapping(string => uint) stringmapping;


    function addToStringMapping(string memory key, uint value) public {
    ...
    function getLocationOfStringMapping(uint mappingSlot, string memory key) public pure returns (bytes32 slot, bytes memory, bytes memory) {
        ...
        return (keccak256(abi.encodePacked(key,mappingSlot)), abi.encode(key, mappingSlot), abi.encodePacked(key,mappingSlot));
